...
class UpdateInsulinEntryUseCase(private val repository: InsulinRepository) {
    suspend operator fun invoke(entry: InsulinEntry) = repository.updateInsulinEntry(entry)
}